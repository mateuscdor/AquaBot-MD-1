const { generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, jidDecode, proto } = require("@adiwajshing/baileys")
...
class ReplyMessage extends Base {
    ...
    async reply(text) {
        var message = await this.client.sendMessage(this.jid, { text: text }, { quoted: message });
        return new Message(this.client, message)
    }

    async sendMessage(text) {
        return await this.client.sendMessage(this.jid, { text: text })
    }
